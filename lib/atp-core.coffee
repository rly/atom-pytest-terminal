...
class ATPCore
  ...
  createDefaultCommandsFile: () ->
    if atom.config.get('atom-pytest-terminal.enableUserCommands')
      try
        initialContent = {
          '_comment': 'Package atom-pytest-terminal: This terminal-commands.json file was automatically generated by atom-terminal-package. It contains all useful config data.'
  ...
  init: () ->
    ...
    try
      ...
      atom.notifications.addWarning "atom-pytest-terminal: Could not load the config file. A default config file will be created. Reason: "+e.message
      @state.opened = false
    if not @state.opened
      @createDefaultCommandsFile()
      @state.opened = true
  ...
  findUserCommandAction: (cmd) ->
    if not atom.config.get('atom-pytest-terminal.enableUserCommands')
      return null
